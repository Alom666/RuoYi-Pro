...
<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" plain icon="el-icon-plus" size="mini" @click="handleAdd" v-hasPermi="['scm:purchaseContract:add']">新建</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="danger" plain icon="el-icon-delete" size="mini" :disabled="multiple" @click="handleDelete" v-hasPermi="['scm:purchaseContract:remove']">删除</el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="contractList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="合同金额" align="center" prop="totalAmount" width="110" />
      ...
      <el-table-column label="操作" align="center" fixed="right" width="150">
        <template slot-scope="scope">
          <el-button size="mini" type="text" icon="el-icon-edit" @click="handleUpdate(scope.row)" v-hasPermi="['scm:purchaseContract:edit']">修改</el-button>
          <el-button size="mini" type="text" icon="el-icon-delete" @click="handleDelete(scope.row)" v-hasPermi="['scm:purchaseContract:remove']">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
        ...
        <el-row :gutter="20">
          <el-col :span="8">
            <el-form-item label="联系人" prop="contactPerson">
              <el-input v-model="form.contactPerson" placeholder="自动回填" />
            </el-form-item>
          </el-col>
          <el-col :span="8">
            <el-form-item label="联系电话" prop="contactPhone">
              <el-input v-model="form.contactPhone" placeholder="自动回填" />
            </el-form-item>
          </el-col>
          <el-col :span="8">
    ...
    <!-- 选择采购订单弹窗 -->
    <el-dialog title="选择采购订单" :visible.sync="openOrderSelect" width="750px" append-to-body>
...
<script>
import { listPurchaseContract, getPurchaseContract, addPurchaseContract, updatePurchaseContract, delPurchaseContract } from '@/api/scm/purchaseContract'
import { listPurchaseApply } from '@/api/scm/purchaseApply'
...
export default {
  ...
  data() {
    return {
      ...
      orderLoading: false,
      orderQueryParams: { applyNo: undefined, pageNum: 1, pageSize: 100, applyStatus: '2' },
      orderList: [],
      // 选择物料弹窗
  ...
  methods: {
    ...
    getList() {
      ...
      listPurchaseContract(params).then(response => {
    ...
    handleAdd() {
      ...
      this.form.contractNo = 'PC-' + y + m + d + '-' + seq
      this.open = true
      this.title = '新增采购合同'
    },
    /** 修改 */
    handleUpdate(row) {
      ...
      getPurchaseContract(contractId).then(response => {
      ...
      this.$refs['form'].validate(valid => {
        if (valid) {
          ...
          if (this.form.contractId != null) {
            updatePurchaseContract(this.form).then(() => {
              this.$modal.msgSuccess(applyStatus === '1' ? '提交成功' : '保存成功')
              this.open = false
              this.getList()
            })
          } else {
            addPurchaseContract(this.form).then(() => {
              this.$modal.msgSuccess(applyStatus === '1' ? '提交成功' : '保存成功')
              this.open = false
              this.getList()
            })
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除采购合同编号为"' + contractIds + '"的数据项？').then(() => {
        return delPurchaseContract(contractIds)
      }).then(() => {
        this.getList()
    ...
    // ========== 选择采购订单（从采购申请中选取已审核的） ==========
    handleSelectOrder() {
      this.orderQueryParams.applyNo = undefined
      this.orderLoading = true
      listPurchaseApply(this.orderQueryParams).then(response => {
        this.orderList = (response.rows || []).map(item => ({
          orderNo: item.applyNo,
          supplierName: item.remark || '-',
          totalAmount: 0,
          deliveryDate: item.applyDate,
    ...
    confirmOrder(row) {
      ...
      this.form.supplierName = row.supplierName !== '-' ? row.supplierName : undefined
      this.form.contactPerson = row.contactPerson
      this.form.contactPhone = row.contactPhone !== '-' ? row.contactPhone : undefined
