...
<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" plain icon="el-icon-plus" size="mini" @click="handleAdd" v-hasPermi="['scm:salesOrder:add']">新建</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="danger" plain icon="el-icon-delete" size="mini" :disabled="multiple" @click="handleDelete" v-hasPermi="['scm:salesOrder:remove']">删除</el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="salesOrderList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="编号" align="center" prop="orderId" width="60" />
      ...
      <el-table-column label="操作" align="center" fixed="right" width="150">
        <template slot-scope="scope">
          <el-button size="mini" type="text" icon="el-icon-edit" @click="handleUpdate(scope.row)" v-hasPermi="['scm:salesOrder:edit']">修改</el-button>
          <el-button size="mini" type="text" icon="el-icon-delete" @click="handleDelete(scope.row)" v-hasPermi="['scm:salesOrder:remove']">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
          ...
          <el-col :span="8">
            <el-form-item label="客户名称" prop="customerName">
              <el-input v-model="form.customerName" placeholder="请选择客户" readonly>
                <el-button slot="append" icon="el-icon-search" @click="handleSelectCustomer"></el-button>
              </el-input>
            </el-form-item>
          </el-col>
          <el-col :span="8">
    ...
    <!-- 选择销售客户弹窗 -->
    <el-dialog title="选择销售客户" :visible.sync="openCustomerSelect" width="700px" append-to-body>
      <el-form :inline="true" size="small">
        <el-form-item label="客户名称">
          <el-input v-model="customerQueryParams.customerName" placeholder="请输入客户名称" clearable @keyup.enter.native="handleCustomerSearch" />
        </el-form-item>
        <el-form-item>
          <el-button type="primary" icon="el-icon-search" @click="handleCustomerSearch">搜索</el-button>
        </el-form-item>
      </el-form>
      <el-table :data="filteredCustomerList" v-loading="customerLoading" height="350" highlight-current-row @current-change="handleCustomerCurrentChange">
        ...
        <el-table-column label="操作" align="center" width="80">
          <template slot-scope="scope">
            <el-button size="mini" type="text" @click="confirmCustomer(scope.row)">选择</el-button>
          </template>
        </el-table-column>
      </el-table>
      <div slot="footer" class="dialog-footer">
        <el-button @click="openCustomerSelect = false">关 闭</el-button>
      </div>
    </el-dialog>
...
<script>
import { listSalesOrder, getSalesOrder, addSalesOrder, updateSalesOrder, delSalesOrder } from '@/api/scm/salesOrder'

export default {
  name: 'SalesOrder',
  dicts: ['sys_apply_status', 'sys_audit_status'],
  data() {
    return {
      loading: true,
      ids: [],
      ...
      detailSelection: [],
      // 选择客户弹窗
      openCustomerSelect: false,
      customerLoading: false,
      customerQueryParams: { customerName: undefined },
      filteredCustomerList: [],
      // 模拟客户数据
      allCustomerList: [
        { customerId: 1, customerName: '华东汽车集团', contactPerson: '张伟', contactPhone: '13800001111' },
        { customerId: 2, customerName: '南方新能源科技', contactPerson: '李娜', contactPhone: '13900002222' },
        { customerId: 3, customerName: '北方重工机械', contactPerson: '王强', contactPhone: '13700003333' },
        { customerId: 4, customerName: '西部动力系统', contactPerson: '赵敏', contactPhone: '13600004444' },
        { customerId: 5, customerName: '中原零部件制造', contactPerson: '陈磊', contactPhone: '13500005555' }
      ],
      // 选择物料弹窗
  ...
  methods: {
    ...
    handleAdd() {
      ...
      this.form.orderNo = 'SO-' + y + m + d + '-' + seq
      this.open = true
      this.title = '新增销售订单'
    },
    /** 修改 */
      ...
      this.$refs['form'].validate(valid => {
        if (valid) {
          ...
          if (this.form.orderId != null) {
            updateSalesOrder(this.form).then(() => {
              this.$modal.msgSuccess(applyStatus === '1' ? '提交成功' : '保存成功')
              this.open = false
              this.getList()
            })
          } else {
            addSalesOrder(this.form).then(() => {
              this.$modal.msgSuccess(applyStatus === '1' ? '提交成功' : '保存成功')
              this.open = false
              this.getList()
            })
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除销售订单编号为"' + orderIds + '"的数据项？').then(() => {
    ...
    handleSelectCustomer() {
      this.customerQueryParams.customerName = undefined
      this.filteredCustomerList = [...this.allCustomerList]
      this.openCustomerSelect = true
    },
    handleCustomerSearch() {
      const name = this.customerQueryParams.customerName
      if (name) {
        this.filteredCustomerList = this.allCustomerList.filter(item => item.customerName.indexOf(name) !== -1)
      } else {
        this.filteredCustomerList = [...this.allCustomerList]
      }
    },
